...
class PredictiveSwarmController(Node):
        ...
        for i, robot_name in enumerate(ALL_ROBOT_NAMES):
            ...
            if dt_prediction > 0 and velocity['timestamp'] > 0:
                ...
                self.get_logger().debug(
                    f"Prédiction {robot_name}: ({predicted_x:.3f}, {predicted_y:.3f}) "
    ...
    def publish_selective_positions_and_velocities(self):
        ...
        for i, robot_name in enumerate(ALL_ROBOT_NAMES):
            # Toujours publier la vitesse actuelle (nécessaire pour la prédiction)
            ...
            if hasattr(self, '_prev_positions'):
                ...
                if dt > 0:
                    ...
                    # Mettre à jour la vitesse locale
            ...
            if self.should_publish_position(i):
                ...
                self.get_logger().info(
                    f"Position publiée pour {robot_name}: ({self.real_positions[i]['x']:.3f}, {self.real_positions[i]['y']:.3f}) "
                    f"[Mise à jour #{self.position_updates_count[i]}]"
                )
    ...
    def timer_callback(self):
        ...
        if self.active and self.formation_initialized and self.goal_point_set:
            ...
            for i, robot_name in enumerate(ALL_ROBOT_NAMES):
                # Position prédite du robot (utilisée pour le contrôle)
    ...
    def apply_consensus_control(self):
        ...
        for i, robot_name in enumerate(ALL_ROBOT_NAMES):
            # Utiliser les positions prédites pour le contrôle
