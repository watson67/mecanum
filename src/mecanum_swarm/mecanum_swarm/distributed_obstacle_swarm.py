...
class DistributedSwarmController(Node):
    ...
    def timer_callback(self):
        ...
        # Mettre à jour la position de l'obstacle via TF2
        ...
        if self._debug_counter % 50 == 0:
            ...
            self.get_logger().info(f"Position obstacle: {self.obstacle_position}")
    ...
    def update_obstacle_position(self):
        ...
        try:
            trans = self.tf_buffer.lookup_transform(
                GLOBAL_FRAME, "obstacle/base_link", rclpy.time.Time(),
                timeout=rclpy.duration.Duration(seconds=0.1)
            )
            pos = trans.transform.translation
