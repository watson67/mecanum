...
class ConsensusSwarm(Node):
    def __init__(self):
        ...
        self.Kp = 0.15                   # Gain proportionnel (réduit de 0.2 à 0.15)
        self.Ki = 0.02                   # Gain intégral (réduit de 0.05 à 0.02)
    ...
    def process_robot(self, robot_name, index):
        ...
        u_gamma = -self.c_gamma * (self.positions[robot_name] - (barycenter + error_to_goal + initial_offset))
        
        # Somme des termes de contrôle
        u_total = u_alpha + u_gamma
        
        # Zone morte: si l'erreur est petite, on ne bouge pas
        position_error = np.linalg.norm(np.array(self.goal_position) - barycenter)
