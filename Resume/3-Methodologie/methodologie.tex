...
Cette stratégie consiste à ne recalculer les deux composantes de commande $u_i^\alpha$ (formation) et $u_i^\gamma$ (attraction) que uniquement lorsque c’est réellement utile. Concrètement, chaque robot surveille deux indicateurs simples : 
\begin{itemize}
    ...
    \]
    \item $\delta_t$ (variation vers la cible) : mise à jour si la variation de la distance à la cible est significative
    \[
        \big|\;\|\mathbf{p}_i-\mathbf{p}_i^*\|-\|\mathbf{p}_i^{\,\text{old}}-\mathbf{p}_i^{*,\,\text{old}}\|\;\big|>\delta_t \;
    \]
