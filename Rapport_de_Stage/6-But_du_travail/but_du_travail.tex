...
Une définition de ce type de système (reconfigurables et modulaires) est donnée dans \cite{seo_modular_2019} : "Les systèmes de robots modulaires reconfigurables (\gls{mrr}) sont constitués de nombreux modules (ou unités) répétés qui peuvent être réorganisés dans différentes configurations en fonction de la tâche que le robot doit accomplir." Une définition similaire est fournie dans \cite{brooks_overview_2021}.
...
\noindent Globalement, le projet eSWARM peut être schématisé comme suit : 
...
La partie recherche porte sur les questions suivantes : comment optimiser la consommation des robots d'un essaim, comment minimiser les échanges d'informations ou encore quel est l'effet des perturbations sur la convergence. L'approche consiste à implémenter différentes méthodes de commande événementielle, puis à injecter des perturbations  pour mesurer la capacité de l'algorithme à résister. 
